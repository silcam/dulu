...
import { FuzzyDateGroup, TextInputGroup } from '../../shared_components/formGroup'
...
class OrgPeopleTableRow extends React.PureComponent {
    ...
    delete = () => {
        ...
        const message = tSub(tSub(this.props.strings.confirm_delete_org_person, subs), subs)
        if (window.confirm(message)) {
            this.props.rawDelete(`/api/organization_people/${orgPerson.id}`, () => {
                const orgPersonIndex = person.organization_people.findIndex((op)=>{return op.id == orgPerson.id})
    ...
    save = () => {
        ...
        this.props.rawPut(url, data, (response) => {
            ...
            const orgPersonIndex = person.organization_people.findIndex((op)=>{return op.id==newOrgPerson.id})
            const newPerson = update(person, {
                organization_people: {
    ...
    render() {
        ...
        if (this.state.editing) {
            return (
                <tr>
                    <td colSpan='4'>
                        <label>{orgPerson.organization.name}</label><br />
                        <TextInputGroup label={strings.Position}
