...
class PublicationTest < ActiveSupport::TestCase
  ...
  test 'Validations' do
    ...
    no_name = Publication.new(kind: 'Bible')
    refute no_name.save, "Shouldn't save without name"
    good_pub = Publication.new(kind: 'Bible', english_name: 'Bible')
    assert good_pub.save, "Should save valid pub"
  end

  test "Name" do
    assert_equal "Ewondo New Testament", @ewondo_nt.name
    french_only = Publication.new(kind: 'Bible', french_name: 'french name')
    nl_only = Publication.new(kind: 'Bible', nl_name: 'nl name')
